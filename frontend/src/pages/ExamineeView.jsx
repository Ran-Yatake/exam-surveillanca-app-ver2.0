    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 max-w-4xl">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-auto text-xs text-slate-600">宛先: 監督者</div>
        </div>
                ...
                <div key={m.id} className="rounded-md border border-slate-200 bg-white p-2">
                  <div className="flex items-center gap-2 text-[11px] text-slate-600">
                    ...
                    {m.fromRole === 'proctor' && m.type === 'broadcast' && (
                      <span className="rounded bg-indigo-600/20 px-2 py-0.5 text-[10px] font-semibold text-indigo-700">
                        一斉送信
                      </span>
                    )}
        ...
        <p className="mt-2 text-[11px] text-slate-500">受験生同士のチャットはできません（監督者⇄受験生のみ）。</p>
      </div>
    </div>
  );
}
